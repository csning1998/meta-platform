...
resource "vault_pki_secret_backend_cert" "listener" {
  ...
  common_name = local.state.network.global_pki_map[local.harbor_pki_role_name].dns_san[0]
  alt_names   = local.state.network.global_pki_map[local.harbor_pki_role_name].dns_san
