# Harbor Origin Recovery for Disk Exhaustion
...
The `provision-harbor-origin-frontend` configuration was modified to disable the aggressive replication schedule defined in `replication.tf`:
...
1. Query the `harbor-db` container inside the bootstrapper host (`core-harbor-origin-frontend-node-00`) to find the exact `user_id`:
