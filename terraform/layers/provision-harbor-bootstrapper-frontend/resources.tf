...
resource "vault_kv_secret_v2" "robot_helm_creds" {
  ...
  mount    = "secret"
  name     = "${local.state.security_vault_approle.vault_kv_namespace}/harbor-bootstrapper/robot"
...
resource "harbor_config_auth" "main" {
  ...
  oidc_client_id     = data.terraform_remote_state.keycloak_oidc.outputs.oidc_clients["harbor-bootstrapper-frontend"].client_id
  oidc_client_secret = data.terraform_remote_state.keycloak_oidc.outputs.oidc_clients["harbor-bootstrapper-frontend"].client_secret
