...
resource "vault_pki_secret_backend_cert" "listener" {
  ...
  name        = "harbor-bootstrapper-frontend"
  common_name = local.state.network.global_pki_map["harbor-bootstrapper-frontend"].dns_san[0]
  alt_names   = local.state.network.global_pki_map["harbor-bootstrapper-frontend"].dns_san
