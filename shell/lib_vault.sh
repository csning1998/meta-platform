...
# PROD_VAULT_ADDR is not declared here: it is derived from the inventory's vault_vip by
# prod_vault_inventory_discoverer (utils_environment.sh) and exported via .env.
...
vault_prod_unseal_trigger() {
  ...
  local inventory_file="${PROD_VAULT_INVENTORY_FILE:-}"
  local playbook_file="${ANSIBLE_DIR}/playbooks/operation_playbook.yaml"

  if [[ -z "$inventory_file" ]]; then
    log_print "ERROR" "PROD_VAULT_INVENTORY_FILE is not set."
    log_print "INFO" "Restart entry.sh to execute prod_vault_inventory_discoverer, or apply shared-vault-frontend if execution has not occurred."
    return 1
  fi

  if [[ ! -f "$inventory_file" ]]; then
    log_print "ERROR" "Production Vault inventory file not found (PROD_VAULT_INVENTORY_FILE='${inventory_file}')."
