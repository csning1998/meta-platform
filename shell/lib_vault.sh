...
readonly DEV_VAULT_ADDR="https://127.0.0.1:8222"
...
vault_status_reporter() {
  ...
  if podman exec -i "${DEV_VAULT_CONTAINER}" vault status -address="${DEV_VAULT_ADDR}" -ca-cert="${DEV_CA}" -format=json > /dev/null 2>&1; then
    local status_json
    status_json=$(podman exec -i "${DEV_VAULT_CONTAINER}" vault status -address="${DEV_VAULT_ADDR}" -ca-cert="${DEV_CA}" -format=json)
    local sealed
    ...
    if podman ps --filter "name=${DEV_VAULT_CONTAINER}" --filter "status=running" | grep -q "${DEV_VAULT_CONTAINER}"; then
...
# Verifies and enables the KV secrets engine for the local development Vault instance.
...
vault_dev_engine_enforcer() {
  ...
  if ! podman exec -i -e VAULT_TOKEN="${root_token}" "${DEV_VAULT_CONTAINER}" vault secrets list -address="${DEV_VAULT_ADDR}" -ca-cert="${DEV_CA}" -format=json | jq -e '."secret/"' > /dev/null; then
    log_print "TASK" "'secret/' path not found, enabling kv-v2..."
    podman exec -i -e VAULT_TOKEN="${root_token}" "${DEV_VAULT_CONTAINER}" vault secrets enable -address="${DEV_VAULT_ADDR}" -ca-cert="${DEV_CA}" -path=secret kv-v2
  else
...
vault_dev_init_handler() {
  ...
  tmp_init=$(mktemp "${DEV_INIT_FILE}.XXXXXX")
	if ! podman exec -i "${DEV_VAULT_CONTAINER}" vault operator init -address="${DEV_VAULT_ADDR}" -ca-cert="${DEV_CA}" -format=json > "$tmp_init"; then
    rm -f "$tmp_init"
    log_print "FATAL" "Initialization failed. Is ${DEV_VAULT_CONTAINER} running?"
    return 1
  fi
  ...
  # Extract Keys
  local tmp_unseal
  tmp_unseal=$(mktemp "${DEV_UNSEAL_KEY_FILE}.XXXXXX")
  chmod 600 "$tmp_unseal"
  if ! jq -r '.unseal_keys_b64[]' "$DEV_INIT_FILE" > "$tmp_unseal"; then
    rm -f "$tmp_unseal"
  ...
  mv "$tmp_unseal" "$DEV_UNSEAL_KEY_FILE"
...
vault_dev_unseal_handler() {
  ...
  local status_json
  status_json=$(podman exec -i "${DEV_VAULT_CONTAINER}" vault status -address="${DEV_VAULT_ADDR}" -ca-cert="${DEV_CA}" -format=json 2>/dev/null || true)
  ...
  while IFS= read -r key; do
    ...
    podman exec "${DEV_VAULT_CONTAINER}" vault operator unseal -address="${DEV_VAULT_ADDR}" -ca-cert="${DEV_CA}" "${clean_key}" || return 1
