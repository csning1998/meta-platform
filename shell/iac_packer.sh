...
packer_build_executor() {
  ...
  if [ -f "${PACKER_DIR}/distro/${base_name}.pkrvars.hcl" ]; then
    sub_dir="distro"
  fi
...
packer_menu_handler() {
  while true; do
    ...
    select category in "${main_options[@]}"; do
      case $category in
        "Base OS Layers")
          if ! packer_layer_selector "distro" "Base OS Images"; then break 2; fi
          break
          ;;
        "Service Layers")
        ...
        "Build ALL")
          ...
          # 1. Build Base Layers
          local base_layers=($(find "${PACKER_DIR}/distro" -name "*.pkrvars.hcl" -printf '%f\n' | sed 's/\.pkrvars\.hcl//g' | sort))
